...
export default function Create() {
  const { data, setData, post, processing, errors } = useForm<FormData>({
    title: '', slug: '', city: '', area: '',
    bedrooms: 0, size_sqft: 0, price: 0,
    status: 'draft', description: '', published_at: ''
  })
    ...
    <div className="p-6 max-w-2xl">
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <input type="number" className="border p-2" placeholder="Bedrooms" value={data.bedrooms} onChange={(e)=>setData('bedrooms', e.target.value)}/>
          <input type="number" className="border p-2" placeholder="Size (sqft)" value={data.size_sqft} onChange={(e)=>setData('size_sqft', e.target.value)}/>
          <input type="number" className="border p-2" placeholder="Price" value={data.price} onChange={(e)=>setData('price', e.target.value)}/>
        </div>
        ...
        <div className="flex gap-2">
          <button disabled={processing} className="px-3 py-2 bg-black text-white rounded">Save</button>
          <Link href="/projects" className="px-3 py-2 border rounded">Cancel</Link>
        </div>
      </form>
    </div>
  )
